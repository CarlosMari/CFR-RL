...
def main(_):
    ...
    # We create a test environment with config parameters
    env = Environment(config, is_training=False)
    game = CFRRL_Game(config, env)

    # We create the neural network with the appropriate dimensions.
    network = Network(config, game.state_dims, game.action_dim, game.max_moves)

    # Restores the network to a given checkpoint
